function results = tracker_DeepRBLT_Scale(params)
SEM_H = [];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Initialization
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
try
    ...
    warning('DeepRBLT:tracker', 'Error when using the mexResize function. Using Matlab''s interpolation function instead, which is slower.\nTry to run the compile script in "external_libs/mexResize/".\n\nThe error was:\n%s', getReport(err));
...
scaleFactors_refine = params.scale_step_refine .^ scale_exp;
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            if scale_ind>1 && scale_ind<7
                refineFactors_1 = [scaleFactors(scale_ind),scaleFactors(scale_ind),scaleFactors(scale_ind),max(scaleFactors_refine(scale_ind-1),scaleFactors(scale_ind-1)),min(scaleFactors(scale_ind+1),scaleFactors_refine(scale_ind+1))];
                refineFactors_2 = [max(scaleFactors_refine(scale_ind-1),scaleFactors(scale_ind-1)),...
                    min(scaleFactors(scale_ind+1),scaleFactors_refine(scale_ind+1)),scaleFactors(scale_ind),scaleFactors(scale_ind),scaleFactors(scale_ind)];
