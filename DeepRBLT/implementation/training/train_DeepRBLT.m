% train DeepRBLT filter
function wf = train_DeepRBLT(params, mf, yf, s,mf_p, wf_p,xf,xf_p,zf)
...
while (iter <= params.admm_iterations)
    wf = cellfun(@(Smy, wf_p, Smm,Smmp, Sxx_fuse,Szz, hf, zetaf,gamma3,gamma1) ...
        bsxfun(@rdivide, ...
		Smy + bsxfun(@times, wf_p, gamma1*Smmp+gamma2*Sxx_fuse)+ mu * hf - zetaf, ...
		(1+gamma1)*Smm + gamma2 * Sxx_fuse+ gamma3 * Szz + mu), ...
		Smy, wf_p, Smm,Smmp, Sxx_fuse,Szz, hf, zetaf,gamma3,gamma1, 'UniformOutput', false);
