function feature_map = extract_features_new(image, pos, scales,scales_2, features, gparams, extract_info)
...
for sz_ind = 1:num_sizes
    ...
    for scale_ind = 1:num_scales
        img_samples{sz_ind}(:,:,:,scale_ind) = sample_patch(image, pos, img_sample_sz.*[scales(scale_ind),scales_2(scale_ind)], img_input_sz, gparams);
    end
end
...
for feat_ind = 1:num_features
    ...
    total_dim = total_dim + sum(features{feat_ind}.fparams.nDim);%41维
end
