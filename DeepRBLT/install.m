...
if exist('external_libs/mtimesx', 'dir') == 7
    ...
    error('DeepRBLT:install', 'Mtimesx not found.')
end
...
if exist('external_libs/pdollar_toolbox/external', 'dir') == 7
    ...
    warning('DeepRBLT:install', 'PDollars toolbox not found. Clone this submodule if you want to use HOG features. Skipping for now.')
end
...
%%%%%% use debug model
%%%%%% vl_compilenn('enableGpu',true,'Debug',true','cudaRoot','C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v11.3','cudaMethod' ,'nvcc','enableCudnn','true','cudnnRoot','C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA')       
        vl_compilenn('enableGpu',true);
    catch err
        warning('DeepRBLT:install', 'Could not compile MatConvNet with GPU support. Compiling for only CPU instead.\nVisit http://www.vlfeat.org/matconvnet/install/ for instructions of how to compile MatConvNet.\nNote: remember to move the mex-files after re-compiling.');
        vl_compilenn;
    end
    ...
    warning('DeepRBLT:install', 'Matconvnet not found. Clone this submodule if you want to use CNN features. Skipping for now.')
end